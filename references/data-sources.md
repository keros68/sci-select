...
Used for journal search, impact factor, latest available CAS partition, SCI/SCIE/ESCI labels, review-speed text, and warning-list hints.

CAS partition note: the official CAS journal partition site states that from 2026 the Chinese Academy of Sciences Documentation and Information Center no longer updates or releases the journal partition table. Do not report "2026 CAS partition". For 2026 onward, use XinRui partition only when checked from a current XinRui source and label it as `新锐分区`.
