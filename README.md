...
- 自带 `assets/sci_select_journals.sqlite` 默认索引，下载后无需额外本地数据即可查询 2025 JIF/JCR Q 区、2025 中科院分区和 2026 新锐分区。
...
仓库不包含原始 Excel、ShowJCR 原始数据库、ShowJCR 项目代码或运行缓存；默认库是 sci-select 自己 schema 生成的 SQLite 文件。当前默认库的 JCR 2025 字段由公开 CSV 导入并规范化为 `jif_2025` / `jcr_quartile_2025`。
...
- `assets/sci_select_journals.sqlite` - 随 skill 发布的默认 sci-select SQLite 索引，包含 2025 JIF/JCR Q 区、2025 中科院分区和 2026 新锐分区。
