...
- 使用公开 LetPub 信息检索期刊候选，并补充 IF、2025 中科院分区、SCI/SCIE/ESCI 标签、审稿速度和预警线索。
- 配置 `XINRUI_API_KEY` 后，自动补充 2026 新锐分区；未配置或查询失败时，在报告中标为 `未获取`。
...
如需自动查询 2026 新锐分区，请先配置新锐 WebAPI Key：
...
选刊质量取决于输入文本完整度、公开网页可访问性、LetPub 页面结构、OpenAlex 匹配质量、新锐 API Key 和宿主 agent 的联网能力。最终投稿前仍建议人工复核期刊官网、最新 JCR、2025 中科院分区、2026 新锐分区、Clarivate Master Journal List 收录状态、版面费、投稿范围、文章类型和撤稿/预警风险。特别注意：不要使用“中科院2026分区”这种表述，当前 SCI/SCIE 状态也不能只依赖第三方缓存。
