...
- 自带 `assets/sci_select_journals.sqlite` 默认索引，下载后无需额外本地数据即可查询 2025 JIF/JCR Q 区、2025 中科院分区、2026 新锐分区和 2026 Nature Index 标记。
- 可选读取用户本地生成的统一 SQLite 索引，用于覆盖或更新内置索引，补充 ISSN、2025 JIF/JCR Q 区、2025 中科院分区、2026 新锐分区和预警标签。
...
- 不打包 ShowJCR 项目源码、ShowJCR 原始 `jcr.db`、原始 Excel 工作簿或运行缓存；仓库内置的是 sci-select 自己 schema 生成的默认 SQLite 索引。
...
仓库自带 `assets/sci_select_journals.sqlite`，安装后无需配置本地数据即可使用。运行时读取顺序是：
...
如果你想更新或覆盖默认数据，可以把你自己整理的 Excel、CSV、ShowJCR 公开 CSV 或 ShowJCR `jcr.db` 一次性导入为 sci-select 自己的 SQLite 索引。Excel 和 ShowJCR 只是导入输入，运行时不再依赖它们。
...
仓库不包含原始 Excel、ShowJCR 原始数据库、ShowJCR 项目代码或运行缓存；默认库是 sci-select 自己 schema 生成的 SQLite 文件。当前默认库的 JCR 2025 字段由公开 CSV 导入并规范化为 `jif_2025` / `jcr_quartile_2025`；Nature Index 字段来自官方 FAQ 当前清单，规范化为 `nature_index` / `nature_index_year` / `nature_index_articles`。
...
识别字段包括 `title`、`issn`、`eissn`、`jif_2025`、`jcr_release_year`、`jcr_data_year`、`jcr_quartile_2025`、`if_2023`、`if_year`、`jcr_quartile`、`cas_2025`、`xuankan_2026`、`nature_index`、`nature_index_year`、`nature_index_articles`、`nature_index_publication_type`、`warning_latest`、`xuankan_warning`、`tags`。如果本地索引和 LetPub 的分区不一致，报告会保留本地索引值，并提示“分区来源冲突需复核”。
...
- `assets/sci_select_journals.sqlite` - 随 skill 发布的默认 sci-select SQLite 索引，包含 2025 JIF/JCR Q 区、2025 中科院分区、2026 新锐分区和 2026 Nature Index 标记。
- `scripts/journal_index_client.py` - 默认/本地/静态期刊索引读取器。
- `scripts/build_journal_index.py` - 从用户自备 Excel、CSV、ShowJCR 公开 CSV 或 ShowJCR SQLite 导入为 sci-select SQLite/JSON 索引。
