...
- 自带 `assets/sci_select_journals.sqlite` 默认索引，下载后无需额外本地数据即可查询 2025 中科院分区和 2026 新锐分区。
...
如果你想更新或覆盖默认数据，可以把你自己整理的 Excel、CSV 或 ShowJCR `jcr.db` 一次性导入为 sci-select 自己的 SQLite 索引。Excel 和 ShowJCR 只是导入输入，运行时不再依赖它们。
...
仓库不包含原始 Excel、ShowJCR 原始数据库、ShowJCR 项目代码或运行缓存；默认库是 sci-select 自己 schema 生成的 SQLite 文件。
...
- `assets/sci_select_journals.sqlite` - 随 skill 发布的默认 sci-select SQLite 索引。
