...
- 已经有目标期刊，想快速查询 IF、最新可得中科院分区/新锐分区、SCI 类型、OA/APC、h-index、审稿速度等信息。
- 想同时看到冲刺、稳妥、保底和谨慎选择，而不是只列高影响因子期刊。
- 需要把候选期刊的 IF、分区、SCI 类型、OA/APC、h-index、审稿速度等信息放在一个报告里。
...
- 使用公开 LetPub 信息检索期刊候选，并补充 IF、最新可得中科院分区、SCI/SCIE/ESCI 标签、审稿速度和预警线索。
...
- 对 2026 年后的分区表达保持区分：中科院文献情报中心已停止更新发布期刊分区表，2026 起不要写“中科院2026分区”；如使用新锐分区，应明确标为 `新锐分区`。
...
- 快速决策表；
- 候选期刊的推荐等级和投稿梯度；
- IF、最新可得分区、SCI 类型、审稿速度、h-index、OA/APC 等指标；
- 主题匹配理由；
...
选刊质量取决于输入文本完整度、公开网页可访问性、LetPub 页面结构、OpenAlex 匹配质量和宿主 agent 的联网能力。最终投稿前仍建议人工复核期刊官网、最新 JCR、最新可用分区、Clarivate Master Journal List 收录状态、版面费、投稿范围、文章类型和撤稿/预警风险。特别注意：2026 年起不再使用“中科院2026分区”这种表述，当前 SCI/SCIE 状态也不能只依赖第三方缓存。
